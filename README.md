...
База данных реализована через  mongodb. Она поднимается отдельным докер контейнером. В файле compose.yml можно увидеть это.
...
Чтобы запустить бота и бд (если хотите, чтобы логи шли в консоль, то вроде нужно опустить флаг -d). И еще следите, чтобы докер был новой версии и поддерживал относительно новую команду compose, а не docker-compose:
```bash
docker compose up -d
```

Чтобы остановить бота и бд:
```bash
docker compose down
```
